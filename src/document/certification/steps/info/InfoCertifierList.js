...
const InfoCertifierList = (props) => {
  ...
  const onCertifierChecked = (certifier, isChecked) => {
    ...
    const newCertifiers = certifiers.map((c) => (
      c.id === newCertifier.id ? ({ ...c, ...newCertifier }) : c));
...
InfoCertifierList.defaultProps = {
  height: 500,
  itemSize: 55,
};

InfoCertifierList.propTypes = {
...
export default InfoCertifierList;
